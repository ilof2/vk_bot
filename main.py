...
def parsing(message, user_id):
	...
	if message.lower() == 'перевыбрать группу' or message.lower()=='начать':
		...
		try:
			...
			return {"keyboard": 
week_day_kbrd.get_body(), "message":"Вы ввели неправильно день недели, используйте сокращенные названия дней недели (пн, вт, ср и т.д.), либо расписание вашей группы еще не добавлено"}

	else:
		# убрано удаление, т.к. сервер может отправить 2 сообщения, и при этом удаляет временный файл, когда это не нужно
		# removing_tmp_file('tmp/'+user_id)
		return {"keyboard": 
groups_spec_kbrd.get_body(), "message": 
"хостинг забыл вашу группу, либо день недели вписан не правильно, пожалуйста,впишите ее заново и используйте сокращенные названия дней (пн, вт, ср и так далее)"}
